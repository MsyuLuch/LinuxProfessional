...
- реализовать это все либо в Vagrant.
...
- `init.sh` - скрипт инициализации для ВМ
Папка build-step включает все те же команды, но разбитые по шагам:
- `init.sh` - скрипт инициализации для ВМ
- `build-1.sh, build-2.sh, build-3.sh, build-4.sh, build-5.sh.sh` - скрипты, которые необходимо запустить после старта ВМ

# **Описание процесса выполнения домашнего задания №6**

Поднимаем виртуальную машину.

Разберем скрипт init.sh:
