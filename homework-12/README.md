...
2:
- `VagrantFile` - файл описывающий виртуальную инфраструктуру для `Vagrant` для второго задания
...
```
В данном случае проблема была в отсутствии разрешающего правила для создания журнала `named.ddns.lab.view1.jnl`, а после для записи в этот файл, при попытке удаленного обновления зоны.
Поэтому решением проблемы было добавление этих правил в Selinux.
Альтернативный, но не предпочтительный, вариант решения проблемы:
```
setenforce 0
```